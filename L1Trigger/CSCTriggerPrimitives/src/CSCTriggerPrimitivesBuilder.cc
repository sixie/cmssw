      ...
      for (int sect = min_sector; sect <= max_sector; sect++)
      {
        for (int subs = min_subsector; subs <= numsubs; subs++)
        {
	      ...
	      put(alctV_all, oc_alct_all, detid, " ALCT digi");
              ...
              //put(clctV_all, oc_clct_all, detid, tmb->getCSCName() + " CLCT digi"); //Sven had this line but Si thinks it was a typo
	      put(clctV_all, oc_clct_all, detid, " CLCT digi"); //Si put this line instead
